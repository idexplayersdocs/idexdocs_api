...
from src.main.composers.link_list_composer import link_list_composer
from src.validators.validate_schema import validate_schema


async def link_list(request: Request):
    try:
        await validate_schema(request)
        http_response = await request_adapter(request, link_list_composer())
