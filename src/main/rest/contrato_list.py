...
from src.main.composers.contrato_list_composer import contrato_list_composer
from src.validators.validate_schema import validate_schema


async def contrato_list(request: Request):
    try:
        await validate_schema(request)
        http_response = await request_adapter(request, contrato_list_composer())
